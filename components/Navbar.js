...
export default function Navbar({show}) {
    ...
    const active = `${inactive} bg-highlight items-center text-black rounded-bl-lg `; //rounded-tr-lg
    const inactiveIcon = 'flex w-6 h-6';
    const activeIcon = `${inactiveIcon} text-primary `; //fill-primary //rounded-tr-lg
    ...
    return (
        <aside className={"text-gray-500 p-4 pr-0 fixed bg-gray-bg h-full min-w-[220px] top-0 md:static md:w-auto transition-all " + (show?'left-0':'-left-full')}> 
            ...
            <nav className="flex flex-col gap-2">
                ...
                {<Link href={'/store'} className={pathname.includes('/store') ? active : inactive}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={pathname.includes('/store') ? activeIcon : inactiveIcon}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" />
                    </svg>
                    Store
                </Link>}
                {isAdmin &&<Link href={'/products'} className={pathname.includes('/products') ? active : inactive}>
                ...
                {isStore && <Link href={'/orders'} className={pathname.includes('/orders') ? active : inactive}>
                ...
                {isStore && <Link href={'/requests'} className={pathname.includes('/requests') ? active : inactive}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={pathname.includes('/requests') ? activeIcon : inactiveIcon}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 8.25V6a2.25 2.25 0 00-2.25-2.25H6A2.25 2.25 0 003.75 6v8.25A2.25 2.25 0 006 16.5h2.25m8.25-8.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-7.5A2.25 2.25 0 018.25 18v-1.5m8.25-8.25h-6a2.25 2.25 0 00-2.25 2.25v6" />
                    </svg>
                    Requests
                </Link>}
                {isAdmin && <Link href={'/categories'} className={pathname.includes('/categories') ? active : inactive}>
